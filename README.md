...
| **Profiles** | Save a reusable implant configuration server-side so it can be referenced later — e.g. by a stage listener — without re-entering the config each time; list/delete saved profiles. **New profile…** opens a dialog exposing the same build options `profiles new` binds: debug, evasion, symbol obfuscation, netgo, run-at-load, and the execution limits (domain-joined, hostname, username, file-exists, locale, not-after). For `shellcode` output it also exposes the [Stagers](https://sliver.sh/docs/?name=Stagers) tuning — encoder (offered per-arch, queried from the server), aPLib compression, and the Windows/Donut-only entropy, exit option, bypass, headers, thread, Unicode and entry-point overrides. Non-Windows shellcode only supports compression, so the Donut fields are hidden for it |
