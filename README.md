...
| **Stage listeners** | Host a saved profile's full implant binary on a staging listener for a stager to pull. **Prepend size** (the 4-byte length prefix Metasploit `custom/*` payloads require) is set on the profile and applied automatically here, and can still be overridden per listener |
...
| **Profiles** | Save a reusable implant configuration server-side so it can be referenced later — e.g. by a stage listener — without re-entering the config each time; list/delete saved profiles. **New profile…** opens a dialog exposing the same build options `profiles new` binds: debug, evasion, symbol obfuscation, netgo, run-at-load, and the execution limits (domain-joined, hostname, username, file-exists, locale, not-after). It also carries **prepend size** for msfvenom/Metasploit stager compatibility — Sliver keeps that on the stage listener rather than the implant, so the bridge stores it against the profile name (`<sliver-root>/webgui-profile-opts.json`) and applies it whenever a stage listener serves that profile. For `shellcode` output it exposes the [Stagers](https://sliver.sh/docs/?name=Stagers) tuning — encoder (offered per-arch, queried from the server), aPLib compression, and the Windows/Donut-only entropy, exit option, bypass, headers, thread, Unicode and entry-point overrides. Non-Windows shellcode only supports compression, so the Donut fields are hidden for it |
